...
# build bitcoind
RUN mkdir ~/source
RUN cd ~/source && wget -O v0.17.1_lightgbt.tar.gz https://github.com/btccom/bitcoin-abc-1/archive/v0.17.1_lightgbt.tar.gz
RUN cd ~/source \
  && tar zxf v0.17.1_lightgbt.tar.gz && cd bitcoin-abc-1-0.17.1_lightgbt \
  && ./autogen.sh \
