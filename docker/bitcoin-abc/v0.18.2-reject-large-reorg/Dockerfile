...
# build bitcoind
RUN mkdir ~/source
RUN cd ~/source && wget -O v0.18.2-reject-large-reorg.tar.gz https://github.com/btccom/bitcoin-abc-1/archive/v0.18.2-reject-large-reorg.tar.gz
RUN cd ~/source \
  && tar zxf v0.18.2-reject-large-reorg.tar.gz && cd bitcoin-abc-1-0.18.2-reject-large-reorg \
  && ./autogen.sh \
