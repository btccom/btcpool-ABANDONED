...
# If the linking error is about `gmp`, run `apt remove libgmp-dev` and rebuild secp256k1, `make install`.
# Then `apt install libgmp-dev` and rebuild BTCPool.

# If `cmake` cannot find a library even if you have installed it, run `rm -rf CMake* cmake*` to clear cache.

apt install -y liblog4cpp5-dev libminiupnpc-dev libleveldb-dev libsnappy-dev libssl-dev libevent-dev libdb++-dev

mkdir /work
cd /work
git clone https://github.com/superbitcoin/secp256k1
cd secp256k1
./autogen.sh
